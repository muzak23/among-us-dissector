...
gameData_type_field 	= ProtoField.int32("amongus.gameData_type" 		, "gameDataType"	, base.DEC)
RPC_type_field  		= ProtoField.int32("amongus.RPC_type" 			, "RPCType"			, base.DEC)
player_color_field		= ProtoField.int32("amongus.player_color" 		, "playerColor"		, base.DEC)

amongus_protocol.fields = { packet_type_field, message_length_field, nonce_field, root_message_type_field, gameID_field, gameData_type_field, RPC_type_field, player_color_field }
...
function amongus_protocol.dissector(buffer, pinfo, tree)
	...
	if packet_type_value == packet_type['RELIABLE'] then
		...
		if root_message_type_value == root_message_type['GameData'] then
			...
			subtree:add(gameData_type_field, gameData_type_value):append_text(" (" .. name(gameData_type, gameData_type_value) .. ")")

            -- If GameData Type: RPC
            if gameData_type_value == gameData_type['RPC'] then
                local RPC_type_value = buffer(14,1):int()
                subtree:add(RPC_type_field, RPC_type_value):append_text(" (" .. name(RPC_type, RPC_type_value) .. ")")

                -- ...

                -- If RPC Type: SetColor
                if RPC_type_value == RPC_type['SetColor'] then
                    local color_value = buffer(19,1):int()
                    subtree:add(player_color_field, color_value):append_text(" (" .. name(player_color, color_value) .. ")")
                end
